...
```shell-session
0xNecr0@htb[/htb]$ nslookup -type=NS zonetransfer.me
...
```shell-session
0xNecr0@htb[/htb]$ nslookup -type=any -query=AXFR zonetransfer.me nsztm1.digi.ninja
...
0xNecr0@htb[/htb]$ export NS="d.ns.facebook.com"
0xNecr0@htb[/htb]$ export WORDLIST="numbers.txt"
0xNecr0@htb[/htb]$ gobuster dns -q -r "${NS}" -d "${TARGET}" -w "${WORDLIST}" -p ./patterns.txt -o "gobuster_${TARGET}.txt"
