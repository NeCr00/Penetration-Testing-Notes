...
```shell-session
0xNecr0@htb[/htb]$ curl -I "http://${TARGET}"
...
```shell-session
0xNecr0@htb[/htb]$ whatweb -a3 https://www.facebook.com -v
...
```shell-session
0xNecr0@htb[/htb]$ sudo apt install wafw00f -y
```
...
```shell-session
0xNecr0@htb[/htb]$ wafw00f -v https://www.tesla.com
...
```shell-session
0xNecr0@htb[/htb]$ sudo apt install golang chromium-driver
0xNecr0@htb[/htb]$ go get github.com/michenriksen/aquatone
0xNecr0@htb[/htb]$ export PATH="$PATH":"$HOME/go/bin"
```
...
```shell-session
0xNecr0@htb[/htb]$ cat facebook_aquatone.txt | aquatone -out ./aquatone -screenshot-timeout 1000
